...
void main() {
    ...
    void main()
    {
        // gl_FragCoord.z contiene el valor de profundidad del fragmento
        // El valor está en el rango [0.0, 1.0]
        float depth = gl_FragCoord.z;

        // Para visualizarlo mejor: 
        // 0.0 (cerca) -> Blanco (1.0)
        // 1.0 (lejos) -> Negro (0.0)
        float color_val = 1.0 - depth;

        // Asignamos el valor de profundidad a los canales R, G y B
        outColor = vec4(vec3(color_val), 1.0);
    }
    """
    ...
    #Matriz perspectiva
    pipeline["projection"] = pyglet.math.Mat4.perspective_projection(WIDTH/HEIGHT, 0.15, 20, 90)
