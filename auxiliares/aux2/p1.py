...
def create_portal(x, y, radio):
    ...
    for i in range(N):
        ...
        # p0
        dtheta0 = i * theta
        x0 = x + radio * np.cos(dtheta0)
        y0 = y + radio * np.sin(dtheta0)
        positions[j+3:j+6] = [x0, y0, 0.0]
     
        # p1
        dtheta1 = dtheta0 + theta
        x1 = x + radio * np.cos(dtheta1)
        y1 = y + radio * np.sin(dtheta1)
...
void main()
{
    ...
    # Creamos el circulo
    circle = create_portal(0.2, 0.0, 0.5)
