...
def create_portal(x, y, radius):
    ...
    dtheta = np.pi / DEFINITION

    for i in range(DEFINITION):
        theta = i*dtheta
        if i%2 == 0:
            positions[i*3:(i+1)*3] = [x + np.cos(theta)*radius, x +radius_lamba + np.cos(theta*2)*radius,y + np.sin(3*theta)*radius ]
        else:
            positions[i*3:(i+1)*3] = [x + radius_lamba + np.cos(theta)*radius, y + np.sin(theta)*radius, x + np.cos(theta*1.5)*radius ]
